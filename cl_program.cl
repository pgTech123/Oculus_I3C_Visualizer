...
//--------------------------------------------------------------------------------------
        ...
        //Go down 1 cube
        else if(level > (numberOfLevels[0]-3)/* || level == (numberOfLevels[0]-1)/*&& stack is empty*/){
            cubeId = getChildId(childId_memStatusBit[cubeId]);
            //debugOutput[0] = cornersArray[cubeId*8];
            //update cubeId, stack, ...
        }
...
{
    //Set memory bit to 0
    atomic_and(childId_memStatusBit+get_global_id(0), 0x1FFFFFFF);
}
